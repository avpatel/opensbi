...
#define EINJ_FFH_TYPE_BIT_MASK          (0xFULL)
#define EINJ_FFH_CHAN_ID_BIT_SHIFT      32
#define EINJ_FFH_CHAN_ID_BIT_MASK       (0x0FFFFFFFULL)
#define EINJ_FFH_REG_ID_BIT_SHIFT       0
#define EINJ_FFH_REG_ID_BIT_MASK        (0xFFFFFFFFULL)

#define EINJ_REG_EXECUTE_OPERATION      RAS_EINJ_READ_REG
#define EINJ_REG_TRIGGER_ERROR          RAS_EINJ_WRITE_REG

#define MAKE_FFH_ADDR(_type, _chan_id, _reg_id)				\
	({								\
		uint64_t _ffh_ = ((((uint64_t)(_type) & EINJ_FFH_TYPE_BIT_MASK) << EINJ_FFH_TYPE_BIT_SHIFT) \
				  | (((uint64_t)(_chan_id) & EINJ_FFH_CHAN_ID_BIT_MASK) << EINJ_FFH_CHAN_ID_BIT_SHIFT) \
				  | (((uint64_t)(_reg_id) & EINJ_FFH_REG_ID_BIT_MASK) << EINJ_FFH_REG_ID_BIT_SHIFT)); \
		(_ffh_);						\
	})
...
static void einj_build_trigger_action_table(void)
{
	...
	inst->register_region.address = MAKE_FFH_ADDR(0, mpxy_chan_id, EINJ_REG_TRIGGER_ERROR);
}
...
static void einj_build_execute_op_entries(void)
{
	...
	inc->inst.register_region.reg_boffs = 0;
	inc->inst.register_region.access_sz = GAS_ACCESS_SZ_QWORD;
	inc->inst.register_region.address = MAKE_FFH_ADDR(0, mpxy_chan_id, EINJ_REG_EXECUTE_OPERATION);
	sbi_list_add(&(inc->node), &(einj_inst_list));
	einj_total_injection_entries += nr_entries;
}
...
int einj_read_reg(uint32_t reg_id, uint64_t *reg_val)
{
	if (!reg_val)
		return SBI_ERR_INVALID_PARAM;

	switch (reg_id) {
	case EINJ_REG_EXECUTE_OPERATION:
	case EINJ_REG_TRIGGER_ERROR:
		*reg_val = 0;
		return SBI_SUCCESS;
	default:
		return SBI_ERR_INVALID_PARAM;
...
int einj_write_reg(uint32_t reg_id, uint64_t reg_val)
{
	(void)reg_val;
