...
__aia_skip:
	...
	/* Detect if hart has AIA IMSIC CSRs */
	if (hfeatures->features & SBI_HART_HAS_AIA) {
		csr_read_allowed(CSR_MCLAIMEI, (unsigned long)&trap);
		if (!trap.cause)
			hfeatures->features |= SBI_HART_HAS_AIA_IMSIC;
	}
}
